...
class SimpleMonitor13(simple_switch_13.SimpleSwitch13):
    ...
    def _monitor(self):
        self.logger.info(
            'time\tdatapath\tin-port\teth-src\teth-dst\tout-port\ttotal_packets\ttotal_bytes')
        while True:
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        for stat in sorted([flow for flow in body if flow.priority == 1],
            ...
            self.logger.info(
                '----------------------------------------------------------------------------------------------')
            self.logger.info(
                'Head\ttime\t\tdatapath\tin-port\teth_src\t\t\teth-dst\t\t\tout_port\ttotal_packet\ttotal_bytes')
            self.logger.info('data\t%s\t%x\t\t%x\t%s\t%s\t%x\t\t%d\t\t%d', self.fields['time'], self.fields['datapath'], self.fields['in-port'],
                             self.fields['eth_src'], self.fields['eth_dst'], self.fields['out-port'], self.fields['total_packets'], self.fields['total_bytes'])
