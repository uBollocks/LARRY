...
proj_location = "/home/larry/Desktop/TORONTO/trafficclassifier/Normal/"
## command to run ##
cmd = "ryu-manager "+proj_location+"normal_monitor.py"
...
class Flow:
    def __init__(self, time_start, datapath, inport, ethsrc, ethdst, outport, packets, bytes):
        #  instance attributes Defination
        ...
        self.forward_packets = int(packets)
        self.forward_bytes = int(float(bytes))
    ...
    def updateforward(self, packets, bytes, curr_time):
        ...
        if (packets <= self.forward_packets or bytes <= self.forward_bytes or  self.forward_delta_bytes == 0 or self.forward_delta_packets == 0):
    ...
    def updatereverse(self, packets, bytes, curr_time):
        ...
        if curr_time != self.reverse_last_time:
            self.reverse_inst_pps = self.reverse_delta_packets / \
                float(curr_time-self.reverse_last_time)
...
def printclassifier(model):
    ...
    for key, flow in flows.items():
        features = np.asarray([flow.forward_delta_packets, flow.forward_delta_bytes, flow.forward_inst_pps,
        flow.forward_avg_pps,flow.forward_inst_bps, flow.forward_avg_bps, flow.reverse_delta_packets,flow.reverse_delta_bytes,
        flow.reverse_inst_pps,flow.reverse_avg_pps,flow.reverse_inst_bps,flow.reverse_avg_bps]).reshape(1,-1) #convert to array so the model can understand the features properly
        # if model is supervised (logistic regression) then the label is the type of traffic
...
def run_ryu(p, traffic_type=None, f=None,model=None):
    ...
    while True:
        ...
        if out != '' and out.startswith(b'data'):
            ...
            try:
                # if Fields list contains: ['1661994441', '1', '', '1', 'c6:d4:25:8d:db:fe', '76:35:38:fc:24:e5', '2', '', '2', '', '140\n']
            ...
            if fields:  # need this otherwise: IndexError: list index out of range
                ...
                if unique_id in flows.keys():
                    # update forward attributes with time, packet, and byte count
                    flows[unique_id].updateforward(
                        int(fields[6]), int(fields[7]), int(fields[0]))
                else:
    ...
    if len(sys.argv) < 2:
        ...
        if len(sys.argv) == 2:
            if sys.argv[1] not in SUBCOMMANDS:
                ...
                print("       Currently subaccepted commands are: %s" %
                      str(SUBCOMMANDS).strip('()'))
                print()
                printHelp()
                sys.exit()
    ...
    if len(sys.argv) == 1:
        ...
        if sys.argv[1] == "train":
            if len(sys.argv) == 3:
                ...
                # open training data output file
                f = open(proj_location+traffic_type+'_training_data.csv', 'w')
            ...
            if sys.argv[1] == 'supervised':
                infile = open(
                    '/home/larry/Desktop/TORONTO/trafficclassifier/Normal/TestLogisticRegression', 'rb')
            elif sys.argv[1] == 'unsupervised':
                infile = open('KMeans_Clustering', 'rb')
            # unload previously trained ML model (refer to Jupyter notebook for details)
